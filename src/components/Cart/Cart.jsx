...
const {numOfCartItems ,totalCartPrice, products , getUpdateCount}=  useContext(cartContext)

console.log("haia", products);

async function handleChangeCount(id ,newCount){
 const res=  await getUpdateCount(id, newCount);

 res ? toast.success("Product is changed") : toast.error("Error during operation, Try again")
}
...
if ( !products){
  ...
  return <>
<h1 className='text-green-950'> Shop Cart:</h1>
<h2 className='text-green-700'> Total cart price: {totalCartPrice}EGP</h2>
